...
``` lua
zheng
├── wms-common -- SSM框架公共模块
├── wms-admin -- 后台管理模板
├── wms-ui -- 前台thymeleaf模板[端口:1000]
├── wms-config -- 配置中心[端口:1001]
├── wms-upms -- 用户权限管理系统
|    ├── wms-upms-common -- upms系统公共模块
|    ├── wms-upms-dao -- 代码生成模块，无需开发
|    ├── wms-upms-client -- 集成upms依赖包，提供单点认证、授权、统一会话管理
|    ├── wms-upms-rpc-api -- rpc接口包
|    ├── wms-upms-rpc-service -- rpc服务提供者
|    └── wms-upms-server -- 用户权限系统及SSO服务端[端口:1111]
├── wms-cms -- 内容管理系统
|    ├── wms-cms-common -- cms系统公共模块
|    ├── wms-cms-dao -- 代码生成模块，无需开发
|    ├── wms-cms-rpc-api -- rpc接口包
|    ├── wms-cms-rpc-service -- rpc服务提供者
|    ├── wms-cms-search -- 搜索服务[端口:2221]
|    ├── wms-cms-admin -- 后台管理[端口:2222]
|    ├── wms-cms-job -- 消息队列、任务调度等[端口:2223]
|    └── wms-cms-web -- 网站前台[端口:2224]
├── wms-pay -- 支付系统
|    ├── wms-pay-common -- pay系统公共模块
|    ├── wms-pay-dao -- 代码生成模块，无需开发
|    ├── wms-pay-rpc-api -- rpc接口包
|    ├── wms-pay-rpc-service -- rpc服务提供者
|    ├── wms-pay-sdk -- 开发工具包
|    ├── wms-pay-admin -- 后台管理[端口:3331]
|    └── wms-pay-web -- 演示示例[端口:3332]
├── wms-ucenter -- 用户系统(包括第三方登录)
|    ├── wms-ucenter-common -- ucenter系统公共模块
|    ├── wms-ucenter-dao -- 代码生成模块，无需开发
|    ├── wms-ucenter-rpc-api -- rpc接口包
|    ├── wms-ucenter-rpc-service -- rpc服务提供者
|    └── wms-ucenter-web -- 网站前台[端口:4441]
├── wms-wechat -- 微信系统
|    ├── wms-wechat-mp -- 微信公众号管理系统
|    |    ├── wms-wechat-mp-dao -- 代码生成模块，无需开发
|    |    ├── wms-wechat-mp-service -- 业务逻辑
|    |    └── wms-wechat-mp-admin -- 后台管理[端口:5551]
|    └── wms-ucenter-app -- 微信小程序后台
├── wms-api -- API接口总线系统
|    ├── wms-api-common -- api系统公共模块
|    ├── wms-api-rpc-api -- rpc接口包
|    ├── wms-api-rpc-service -- rpc服务提供者
|    └── wms-api-server -- api系统服务端[端口:6666]
├── wms-oss -- 对象存储系统
|    ├── wms-oss-sdk -- 开发工具包
|    ├── wms-oss-web -- 前台接口[端口:7771]
|    └── wms-oss-admin -- 后台管理[端口:7772]
├── wms-shop -- 电子商务系统
└── wms-demo -- 示例模块(包含一些示例代码等)
     ├── wms-demo-rpc-api -- rpc接口包
     ├── wms-demo-rpc-service -- rpc服务提供者
     └── wms-demo-web -- 演示示例[端口:8888]
```
...
![统一扫码支付](project-bootstrap/wms-pay.png)
...
- 修改各dao模块和rpc-service模块的redis.properties、jdbc.properties、generator.properties数据库连接等配置信息，其中master.redis.password、master.jdbc.password、slave.jdbc.password、generator.jdbc.password密码值使用了AES加密，请使用org.suvan.common.util.AESUtil工具类修改这些值

- 启动Zookeeper、Redis、ActiveMQ、Nginx（配置文件参考project-tools/nginx下的*.conf文件）

> **wms-upms**

- 首先启动 wms-upms-rpc-service(直接运行src目录下的ZhengUpmsRpcServiceApplication#main方法启动) => wms-upms-server(jetty)，然后按需启动对应子系统xxx的wms-xxx-rpc-service(main方法) => wms-xxx-webapp(jetty)

![启动演示](project-bootstrap/start.png)

- 访问 [http://upms.zhangshuzheng.cn:1111/](http://upms.zhangshuzheng.cn:1111/ "统一后台地址")，子系统菜单已经配置到wms-upms权限中，不用直接访问子系统，默认帐号密码：admin/123456

- 登录成功后，可在右上角切换已注册系统访问

> **wms-cms**

- wms-cms-admin：启动ActiveMQ-启动 => 启动wms-rpc-service => 启动wms-cms-admin

- wms-cms-web：启动nginx代理wms-ui静态资源，配置文件可参考 [nginx.conf](http://git.oschina.net/shuzheng/zheng/attach_files)

> **wms-oss**

- 首先启动wms-oss-web服务

- 开发阶段，如果wms-oss-web没有公网域名，推荐使用`ngrok`内网穿透工具，为开发环境提供公网域名，实现上传回调
...
- rpc-service服务提供者jar包：将打包后的wms-xxx-rpc-service-assembly.tar.gz文件解压，使用bin目录的管理脚本运行即可，支持优雅停机。
...
- 模块命名为`项目`-`子项目`-`业务`，如`wms-cms-admin`
...
演示地址： [http://upms.zhangshuzheng.cn/](http://47.93.195.63/wms-upms-server/sso/login?backurl=http%3A%2F%2F47.93.195.63%2Fwms-upms-server%2Fmanage%2Findex "演示地址")

### 预览图
![idea](project-bootstrap/idea.png)
![login](project-bootstrap/wms-login.png)
![upms](project-bootstrap/wms-upms.png)
![cms](project-bootstrap/wms-cms.png)
...
- 报wms-xxx.jar包找不到,请按照文档编译顺序，将源代码编译并安装到本地maven仓库

- wms-cms-admin启动卡住：因为没有启动activemq

- wms-upms-server访问报session不存在：因为没有启动redis服务

- 界面没有样式：因为wms-admin没有编译安装到本地仓库
...
- [wms-1：环境搭建及项目部署](http://www.jianshu.com/p/b2fb42e17b58 "wms-1：环境搭建及项目部署")
...
- [进击zheng项目wms-umps-server](http://blog.csdn.net/u014449653/article/details/78355849 "进击zheng项目wms-umps-server")

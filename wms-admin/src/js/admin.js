...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('wms-upms-systemid', systemid);
		$.cookie('wms-upms-systemname', systemname);
		$.cookie('wms-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('wms-upms-systemid') || 1;
	var systemname = $.cookie('wms-upms-systemname') || 'wms-upms-server';
	var systemtitle = $.cookie('wms-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);

package org.suvan.upms.client.shiro.filter;

import com.alibaba.fastjson.JSONObject;
import org.suvan.common.util.PropertiesFileUtil;
import org.suvan.common.util.RedisUtil;
import org.suvan.upms.client.shiro.session.UpmsSessionDao;
import org.suvan.upms.client.util.RequestParameterUtil;
import org.suvan.upms.common.constant.UpmsConstant;
...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "wms-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "wms-upms-client-session-ids";
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("wms-upms-client").get("wms.upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer ssoServerUrl = new StringBuffer(PropertiesFileUtil.getInstance("wms-upms-client").get("wms.upms.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("wms-upms-client").get("wms.upms.type");
        if ("server".equals(upmsType)) {
        ...
        ssoServerUrl.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("wms-upms-client").get("wms.upms.appID"));
        // 回跳地址
